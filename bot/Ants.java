...
public final class Ants {
     ...
     */
    public void setTurnStartTime(final long turnStartTime) {
    ...
    public long getTimeRemaining() {
        return turnTime - (System.nanoTime() - turnStartTime);
    }

    /**
     * Returns elapsed time in current turn.
     *
     * @return elapsed time in current turn
     */
    ...
    /**
     * Calculates Manhattan distance between two locations on the game map.
     *
     * @param t1 one location on the map
     * @param t2 another location on the map
     * @return Manhattan distance between <code>t1</code> and <code>t2</code>
     */
    public int getManhattanDistance(final Tile t1, final Tile t2) {
        int rowDelta = Math.abs(t1.getRow() - t2.getRow());
        int colDelta = Math.abs(t1.getCol() - t2.getCol());
        rowDelta = Math.min(rowDelta, rows - rowDelta);
        colDelta = Math.min(colDelta, cols - colDelta);
    ...
    /**
     * Calculates row distance between two locations on game map.
     *
     * @param t1 one location on the map
     * @param t2 another location on the map
     * @return row distance between <code>t1</code> and <code>t2</code>
     */
    public int getRowDelta(final Tile t1, final Tile t2) {
        int rowDelta = Math.abs(t1.getRow() - t2.getRow());
        return Math.min(rowDelta, rows - rowDelta);
    }

    /**
     * Calculates column distance between two locations on game map.
     *
     * @param t1 one location on the map
     * @param t2 another location on the map
     * @return column distance between <code>t1</code> and <code>t2</code>
     */
    public int getColDelta(final Tile t1, final Tile t2) {
        int colDelta = Math.abs(t1.getCol() - t2.getCol());
    ...
    /**
     * Lists adjacent locations for a location on the map.
     *
     * @param tile a location on the map
     * @return adjacent locations
     */
    List<Tile> getNeighbours(final Tile tile) {
        List<Tile> neighbours = new LinkedList<Tile>();
        for (Aim direction : Aim.values()) {
            neighbours.add(getTile(tile, direction));
        }
        return neighbours;
    }
